    ...
    <p>
        @if (\Auth::check())
            ...
            <form action="{{ route('courses.payment') }}" method="POST">
                ...
                <input type="hidden" name="amount" value="{{ number_format($course->price/100) * 100 }}" />
                <script
                    src="https://checkout.stripe.com/checkout.js" class="stripe-button"
                    data-key="pk_test_QOMi4IVFedR5HRerKIXn1bkQ"
                    data-amount="{{ number_format($course->price/100) * 100 }}"
                    data-currency="usd"
                    data-name="Quick LMS"
                    data-label="Buy course (&#8358;{{ number_format($course->price/100) }})"
